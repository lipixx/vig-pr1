...
void Object::Render(bool suavitzat)
{
  CalculaNormals(); 
  Material m;
  ...
  for (uint i=0;i<faces.size();i++)
	...
	mètode material(int), que retorna el material associat
      ...
      */
      ...
      //const GLfloat* shininess = (GLfloat) m.shininess;
	...
	//vèrtexs de Object::vertices<Vertex>.	  
	  if(suavitzat)
          {
	    glNormal3f(vertices[faces[i].vertices[j]].normal.x,vertices[faces[i].vertices[j]].normal.y,vertices[faces[i].vertices[j]].normal.z); 
	  }
...
void Object::CalculaNormals()
{
 for(unsigned int i=0; i<faces.size(); ++i)
 {
      ...
      //Afegit al codi donat...
       for(unsigned int j=0; j<faces[i].vertices.size(); j++)
       {
               vertices[faces[i].vertices[j]].normal += faces[i].normal;
               vertices[faces[i].vertices[j]].numFaces++;
       }
 }
 for(uint n = 0; n < vertices.size(); n++)
       if (vertices[n].numFaces) vertices[n].normal = vertices[n].normal/vertices[n].numFaces;
}
